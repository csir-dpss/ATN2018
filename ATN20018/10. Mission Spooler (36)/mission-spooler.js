...
socket.on('0E00h', (nodeInfo, acknowledge) => {
    console.log('nodeinfi from the ocu', nodeInfo)
    console.log(`\n => ${nodeInfo.sender.name} is requesting to start the mission ${nodeInfo.data[0].name}`);
...
socket.on('0E03h', (nodeInfo, acknowledge) => {
    ...
    socket.emit('0E03h', nodeInfo, (ack) => {
      console.log(JSON.stringify(ack, null, 4));
      console.log("sent the pause mission command to the sub-system");
  });
});
