...
// 1.15 Acknowledgement : Message ID = 4400h
  ...
  socket.on('2A22h', (nodeInfo, acknowledge) => {
    ...
    console.log('getting repient from the communicator name', sendTo)
    // retrieving its socket id  
    var destination = sentTo[0].socketID;
  ...
  socket.on('2A23h', (nodeInfo, acknowledge) => {
    console.log('getting data from nodeInfo', nodeInfo)
  // console.log(`\n => ${nodeInfo.sender.name} is Querying Vector Knowledge Store Object from ${nodeInfo.data[0].from}`);
    ...
    // checking if recipient is still connected 
    var sentTo = systemTree.filter((node) => {
      return node.name === nodeInfo.recipient;
    });
  ...
  socket.on('4A23h', (nodeInfo, acknowledge) => {
    console.log('4A node info',nodeInfo )
    console.log(`\n => ${nodeInfo.sender.name} is Reporting Vector Knowledge Store Object to ${nodeInfo.recipient}`);
    ...
    console.log('getting all the data sentTo', sentTo)
    // retrieving its socket id  
    var destination = sentTo[0].name;
  ...
  socket.on('0E00h', (nodeInfo, acknowledge) => {
    ...
    console.log('this is the nodeinfo', nodeInfo)
  ...
  socket.on('0E01h', (nodeInfo, acknowledge) => {
    ...
    ack.timestamp = moment().valueOf();
    var nack = setTimeout(() => { console.log('Timed out!') }, 1000);  // do something about this
    socket.emit(nodeInfo.messageID, nodeInfo);
   // acknowledge(ack);
    clearTimeout(nack);
  });
  ...
  socket.on('0E02h', (nodeInfo, acknowledge) => {
    console.log(`\n => ${nodeInfo.sender.name} is sending an Stop Mission Command to  ${nodeInfo.recipient.name}`);
  ...
  socket.on('0E04h', (nodeInfo, acknowledge) => {
    console.log(`\n => ${nodeInfo.sender.name} is sending a Resume Mission Command to ${nodeInfo.recipient.name}`);
