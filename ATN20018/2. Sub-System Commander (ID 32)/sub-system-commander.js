...
socket.on('0E02h', (nodeInfo, acknowledge) => {
  console.log(`\n => ${nodeInfo.sender.name} is requesting a stop to ${nodeInfo.recipient.name}`);
...
socket.on('0E00h', (nodeInfo, acknowledge) => {
  console.log('getting all the data', nodeInfo.data)
  console.log(`\n => ${nodeInfo.sender.name} is starting the mission ${nodeInfo.data[0].name}`);
  var ack = {
    command: '0403h'
  };
  socket.emit('4400h', nodeInfo, (ack) => {
  ...
  console.log('recepitent', destNodeInfo[0].name)
  if (destNodeInfo.length === 0) { // if not connected
    console.log(`\n => The ${destNodeInfo[0].name} is not connected to the G-Bat Network!`);
  } else { // if connected
    // 1.3. Formatting the packet to be sent
    console.log('messages before sending all the data ...... ')
    var nodeInfo = {
      messageID: messageID,
      sender: me,
      recipient: destNodeInfo[0].name,
      data: nodeInfo.data,
      sequenceNo: sequence,

    };
    console.log('messages before sending all the data ...... ', nodeInfo)
    //1.4 sending the message
    console.log(`\n\n -> Sending ${nodeInfo.messageID} command to ${nodeInfo.recipient}....`);
